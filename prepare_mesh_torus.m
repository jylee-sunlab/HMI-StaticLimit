function outputFile = prepare_mesh_torus(gmshExecutable)
% Usage
%   outputFile = prepare_mesh_torus();
%   outputFile = prepare_mesh_torus('\path\to\gmsh.exe');
%
% Requirements
...
tubeRadius = (2/(9*pi))^(1/3)*a;
majorRadius = 3*tubeRadius;
outerRadius = 2.2*a;
...
outputFile = fullfile(pwd,'mesh_torus.mat');
workDir = fullfile(pwd,'mesh_torus_work');
% =====================================================================
% END USER-EDITABLE GEOMETRY
% =====================================================================

if majorRadius+tubeRadius >= outerRadius
    error('HMIDemo:Geometry', ...
        'outerRadius must enclose the entire torus.');
end

if portRadius >= tubeRadius
    error('HMIDemo:Geometry', ...
        'portRadius must be smaller than the torus tube radius.');
end

for p=1:size(portCenters,1)
    rho = hypot(portCenters(p,1),portCenters(p,2));
    distToCenterline = hypot(rho-majorRadius,portCenters(p,3));
    if distToCenterline+portRadius >= tubeRadius
        error('HMIDemo:Geometry', ...
            'Port %d is not strictly inside the torus.',p);
    end
end
...
geoFile = fullfile(workDir,'mesh_torus.geo');
mshFile = fullfile(workDir,'mesh_torus.msh');

demo_write_torus_geo( ...
    geoFile,a,majorRadius,tubeRadius,outerRadius, ...
...
rho = hypot(centroid(:,1),centroid(:,2));
torusDistance = sqrt((rho-majorRadius).^2+centroid(:,3).^2);

solidMask = torusDistance <= tubeRadius*(1+1e-10);
fluidMask = ~solidMask;
...
for p=1:size(portCenters,1)
    ...
    if any(hit & ~solidMask)
        error('HMIDemo:PortOutsideSolid', ...
            'Port %d contains elements outside the torus.',p);
    end
...
model.meta = struct();
model.meta.geometry = 'torus';
model.meta.majorRadius = majorRadius;
model.meta.tubeRadius = tubeRadius;
...
fprintf('\nTorus mesh prepared.\n');
fprintf('  major radius/a     %.8g\n',majorRadius/a);
fprintf('  tube radius/a      %.8g\n',tubeRadius/a);
fprintf('  nodes              %d\n',size(X,1));
...
% Gmsh geometry
% =====================================================================
function demo_write_torus_geo( ...
    filename,a,majorRadius,tubeRadius,outerRadius, ...
    portCenters,portRadius,hPort,hSolid,hFluid)
...
fprintf(fid,'Torus(2) = {0,0,0,%.17g,%.17g};\n', ...
...
padSolid = 0.10*a;
extentXY = majorRadius+tubeRadius+padSolid;
extentZ = tubeRadius+padSolid;
