function outputFile = prepare_mesh_sphere(gmshExecutable)
% Usage
%   outputFile = prepare_mesh_sphere();
%   outputFile = prepare_mesh_sphere('\path\to\gmsh.exe');
%
% Requirements
...
outputFile = fullfile(pwd,'mesh_sphere.mat');
workDir = fullfile(pwd,'mesh_sphere_work');
% =====================================================================
% END USER-EDITABLE GEOMETRY
% =====================================================================

% Basic geometry
if outerRadius <= solidRadius
    error('HMIDemo:Geometry','outerRadius must exceed solidRadius.');
end
for p=1:size(portCenters,1)
    if norm(portCenters(p,:))+portRadius >= solidRadius
        error('HMIDemo:Geometry','Port %d is not strictly inside the solid.',p);
    end
end
for p=1:size(portCenters,1)
    for q=p+1:size(portCenters,1)
        if norm(portCenters(p,:)-portCenters(q,:)) <= 2*portRadius
            error('HMIDemo:Geometry','Ports %d and %d overlap.',p,q);
        end
    end
end
...
geoFile = fullfile(workDir,'mesh_sphere.geo');
mshFile = fullfile(workDir,'mesh_sphere.msh');

demo_write_sphere_geo(geoFile,a,solidRadius,outerRadius, ...
    portCenters,portRadius,hPort,hSolid,hFluid);
...
X = raw.nodes;
T = raw.tet4;
T = demo_orient_tet4(X,T);
centroid = (X(T(:,1),:) + X(T(:,2),:) + X(T(:,3),:) + X(T(:,4),:))/4;

rSolid = sqrt(sum(centroid.^2,2));
solidMask = rSolid <= solidRadius*(1+1e-10);
fluidMask = ~solidMask;
...
for p=1:size(portCenters,1)
    ...
    if any(hit & portIndex>0)
        error('HMIDemo:PortOverlap','Port %d overlaps a previous port.',p);
    end
    portIndex(hit)=p;
end
if any(portIndex>0 & ~solidMask)
    error('HMIDemo:PortClassification','A port element is not solid.');
end
if ~any(solidMask) || ~any(fluidMask)
    error('HMIDemo:RegionClassification','Solid or fluid classification is empty.');
end
for p = 1:size(portCenters,1)
    if ~any(portIndex==p)
        error('HMIDemo:EmptyPort','Port %d contains no tetrahedra.',p);
    end
end

tri = raw.tri3;
rv = [sqrt(sum(X(tri(:,1),:).^2,2)), ...
...
if isempty(outerFaces)
    error('HMIDemo:OuterBoundary','Could not identify the outer boundary faces.');
end
...
model.ports = repmat(struct('elements',[],'nominalCenter',[]), ...
...
model.meta = struct();
model.meta.geometry = 'test sphere';
...
fprintf('\nLightweight test mesh prepared.\n');
fprintf('  nodes              %d\n',size(X,1));
...
% Gmsh geometry
% =====================================================================
function demo_write_sphere_geo(filename,a,solidRadius,outerRadius, ...
    portCenters,portRadius,hPort,hSolid,hFluid)
...
for p=1:size(portCenters,1)
    c=portCenters(p,:);
    fprintf(fid,'Sphere(%d) = {%.17g,%.17g,%.17g,%.17g};\n', ...
...
fprintf(fid,'Field[1].VIn = %.17g; Field[1].VOut = %.17g;\n',hSolid,hFluid);
fprintf(fid,'Field[1].XMin = %.17g; Field[1].XMax = %.17g;\n',-1.15*a,1.15*a);
fprintf(fid,'Field[1].YMin = %.17g; Field[1].YMax = %.17g;\n',-1.15*a,1.15*a);
fprintf(fid,'Field[1].ZMin = %.17g; Field[1].ZMax = %.17g;\n',-1.15*a,1.15*a);
...
for p=1:size(portCenters,1)
    ...
    fprintf(fid,'Field[%d].VIn = %.17g; Field[%d].VOut = %.17g;\n',tag,hPort,tag,hSolid);
    fprintf(fid,'Field[%d].XMin = %.17g; Field[%d].XMax = %.17g;\n',tag,c(1)-pad,tag,c(1)+pad);
    fprintf(fid,'Field[%d].YMin = %.17g; Field[%d].YMax = %.17g;\n',tag,c(2)-pad,tag,c(2)+pad);
    fprintf(fid,'Field[%d].ZMin = %.17g; Field[%d].ZMax = %.17g;\n',tag,c(3)-pad,tag,c(3)+pad);
end
minTag=20;
flist=sprintf('%d,',fieldTags);
flist(end)=[];
fprintf(fid,'Field[%d] = Min; Field[%d].FieldsList = {%s};\n',minTag,minTag,flist);
...
candidates=[candidates,{'gmsh',fullfile(pwd,'gmsh.exe'),'C:\Program Files\gmsh\gmsh.exe'}];
...
while ~feof(fid)
    ...
    if strcmp(line,'$Nodes')
        ...
        assert(strcmp(strtrim(fgetl(fid)),'$EndNodes'),'Malformed $Nodes section.');
    elseif strcmp(line,'$Elements')
        ...
        assert(strcmp(strtrim(fgetl(fid)),'$EndElements'),'Malformed $Elements section.');
    end
end
if isempty(nodes) || isempty(tetTags)
    error('HMIDemo:MeshFormat','No tetrahedra found.');
end
maxTag=max(nodeTags);
map=zeros(maxTag,1);
